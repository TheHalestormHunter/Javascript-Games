...
function animate1(){
	...
	var reset = setTimeout(resetAnimation, 300);
}
...
function attack(value){
	console.log("click")
	if(pokemon[0].stats[3] > pokemon[1].stats[3]){
		moveNum = parseInt(value);
		aiAttacked = false;
		pokeAttack();
	} else if(pokemon[0].stats[3] < pokemon[1].stats[3]){
		aiAttack();
		moveNum = parseInt(value);
		aiAttacked = true;
		var delay = setTimeout(pokeAttack,500)
	}
}
...
function displayMsg(text){
	msgArray.push(text);
	var delay = setTimeout(update, 150);
}
...
function hit(){
	...
	if(pokemon[caller].moveset[moveNum].category.includes("Status") == true){
	...
	if(repeatValue > 1){
		while(repeatValue != -1){
			...
			var delay = setTimeout(function repeater(){
...
function valueText(p){
	var limit;		
	for(i=0; i<5; i++){
		if(pokemon[p].statStage[i] > 12){
			pokemon[p].statStage[i] = 12;
			limit = true;
		}
		pokemon[p].baseStage[i] = pokemon[p].statStage[i];
	}
	if(!limit){
		if(valueChange < 0){
			text2 = " fell";
...
function dmgTypeCalc(){
	for(var type of pokemon[caller].moveset[moveNum].type){
		...
		for(var weakness of weaknessTo[type]){
			console.log("weakeness " + weakness)
			if(pokemon[target].type.includes(weakness) == true){
				typeEff = 2;
				console.log(weakness + " is weak to " + type)
			} else typeEff = 1;
		}
		for(var resistant of resistanceTo[type]){
			console.log("resistant " + resistant)
			if(pokemon[target].type.includes(resistant) == true){
				console.log(resistant + " is resistant to " + type)
				typeEff = .5;
			} else typeEff = 1;
		}
    	for(var immune of immuneTo[type]){
			...
			if(pokemon[target].type.includes(immune) == true){
				console.log(immune + " is immune to " + type)
				typeEff = 0;
  				} else typeEff = 1;
			} 
