...
function savePoke(){
	...
	localStorage.setItem("id1", id1);
	localStorage.setObj("savedMoves", pokemon[0].moveset);
...
function expCalc(){
	if(pokemon[0].expType.includes("Fast") == true){
		pokemon[0].expLvl = Math.abs((Math.pow((4*pokemon[0].level), 3)) / 5);
		// next level exp = 4n^3 / 5
	}
	else if(pokemon[0].expType.includes("Medium Fast") == true){
		pokemon[0].expLvl = Math.abs(Math.pow(pokemon[0].level, 3));
		// n^3
	}
	else if(pokemon[0].expType.includes("Medium Slow") == true){
		pokemon[0].expLvl = Math.abs((Math.pow(((6/5)*pokemon[0].level), 3)) - (Math.pow((15*pokemon[0].level), 2)) + (100*pokemon[0].level) - 140);
		//  5/6n^3 - 15n^2 + 100n - 140
	}
	else if(pokemon[0].expType.includes("Slow") == true){
		pokemon[0].expLvl = Math.abs(Math.pow((5*pokemon[0].level), 3) / 4);
		// 5n^3/4
	}
}

function expGain(){
	gain = (pokemon[1].level * pokemon[1].expYield) * 10
	...
	if(pokemon[1].currentHealth > 0){
		if(pokemon[0].moveset[moveNum].PP >=1){
		    ...
		    if(pokemon[0].moveset[moveNum].accuracy >= accuracy){
				...
				if(damage == 0){
					document.getElementById("desc").innterHTML = pokemon[0].moveset[moveNum].name + " has no effect on " + pokemon[1].name;
				}
				else {
					...
					log();
					animate1();
					document.getElementById("desc").innerHTML = pokemon[0].name + " used " + pokemon[0].moveset[moveNum].name + "!"
				}
			} else {
				document.getElementById("desc").innerHTML = pokemon[0].name + "'s " +pokemon[0].moveset[moveNum].name + " missed!"
			}
			if(pokemon[1].currentHealth < 0){
				document.getElementById("desc").innerHTML = pokemon[1].name + " fainted!";
				evCalc();
				expGain();
			}
			else {
...
function aiAttack(){
    ...
    if(pokemon[1].moveset[moveNum].accuracy >= accuracy){
		...
		console.log("the damage was " + damage)
		if(damage == 0){
			document.getElementById("desc").innterHTML = pokemon[1].moveset[moveNum].name + " has no effect on " + pokemon[0].name;
		}
		else {
			...
			log();
			animate2();
			document.getElementById("desc").innerHTML = "Enemy " + pokemon[1].name + " used " + pokemon[1].moveset[0].name + "!"
		}
	} else {
		document.getElementById("desc").innerHTML = pokemon[1].name + "'s " +pokemon[1].moveset[0].name + " missed!"
	}
	if(pokemon[0].currentHealth < 0){
		document.getElementById("desc").innerHTML = pokemon[0].name + " fainted!";
	}	
	enableButtons()
}
